...
class BinaryAM(eqx.Module):
    ...
    def async_recall(
        ...
        # Apply `async_update` to each bitflip in deq
...
video = show_cached_recall_animation(cache_name, steps_per_sample=32)
video
```
...
We'll implement a `logsumexp` version of the DenseAM energy function that is numerically stable. This energy function was introduced by [@ramsauer2021hopfield] who called it the "Modern Hopfield Network" (MHN), and it can be understood as an exponential form of the DenseAM [@demicirgil2017model].
...
Xi = data[:1024]
fig1, ax1 = show_im(jr.choice(jr.PRNGKey(10), Xi, shape=(255,), replace=False));
```
...
x_noisy = flip_some_bits(key2, x_og, 0.2)
...
if os.path.exists(npz_fname) and CACHE_RECALL:
    ...
    np.savez(cache_name + '.npz', x_final=x_final, frames=frames, energies=energies)
```
...
def show_batched_recall_output(x_og, x_noisy, x_final, energies):
    ...
    show_im(x_og, axes[0]); axes[0].set_title("Original patterns")
    show_im(x_noisy, axes[1]); axes[1].set_title("Noisy queries") 
    show_im(x_final, axes[2]); axes[2].set_title("Retrieved patterns")
    
    # Plot all energy curves
    for i in range(energies.shape[0]):
        axes[3].plot(energies[i], alpha=0.7)
    axes[3].set_title("Energy during recall (all)")
...
def show_batched_recall_animation(frames, energies, cache_name, steps_per_sample=64):
    ...
    fname_gif = cache_name + '.gif'
    imageio.mimsave(fname_gif, images, duration=0.1)
    
    return Image(filename=fname_gif, width=400), fname_gif

if os.path.exists(cache_name + '.gif') and CACHE_RECALL:
    gif = Image(filename=cache_name + '.gif', width=400)
else:
