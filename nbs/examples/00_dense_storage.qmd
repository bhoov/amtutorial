...
```{python}
data = np.load("data/processed_pokemon_sprites_adaptive/pokesprites_pixels.npy")
# data = np.load("data/processed_pokemon_sprites_dithered_0.8/pokesprites_pixels.npy")
data = (data / 255.) * 2 - 1 # Binarize

# Assume square data
pxh = pxw = data.shape[1]
...
x_final, frames, energies = cached_recall(chn, 'hopfield_recovery_data.npz', x_noisy, nsteps=12000, key=jr.PRNGKey(5))

def show_recall_output(x_og, x_noisy, x_final, energies):
    fig, axes = plt.subplots(1, 4, figsize=(16, 4)) # Changed to 1 row, 4 columns
    show_im(x_og, axes[0]); axes[0].set_title("Original pattern")
    show_im(x_noisy, axes[1]); axes[1].set_title("Noisy Query")
    show_im(x_final, axes[2]); axes[2].set_title("Retrieved pattern")
    
    # Add energy plot
    axes[3].plot(energies)
    axes[3].set_title("Energy during Recall")
...
We can animate the recall process to view the "thinking" process of the CHN. (Please forgive the blurriness of the gif. Browsers automatically interpolate when small images are upscaled.)
...
def show_recall_animation(frames, fname, steps_per_sample=32):
    ...
    return Image(filename=fname, width=pxh * 5)
...
def show_cached_recall_animation(fname, steps_per_sample=32):
    ...
    if SHOW_FULL_ANIMATIONS:
        display(show_recall_with_energy_animation(frames, energies, fname, steps_per_sample=steps_per_sample, force_remake=(not CACHE_RECALL)))
    else:
        display(show_recall_animation(frames, fname, steps_per_sample=steps_per_sample))

show_cached_recall_animation('hopfield_recovery_data.npz', steps_per_sample=32)
```
...
show_cached_recall_animation(fname, steps_per_sample=32)
```
...
A simple change to using a polynomial of degree $12$ instead of the CHN's quadratic energy function allows us to store retrieve our desired patterns, even up to $K=250$ patterns!

```{python}
# DenseAM: Increase the number of stored patterns!
Xi = jr.choice(mem_key, data, shape=(250,))
fig1, ax1 = show_im(Xi);
ax1.set_title("Stored patterns")
dam = DenseAM(Xi, n=12, rectified=True)

fname = f'dam_recovery_n={dam.n}_K={Xi.shape[0]}.npz'
