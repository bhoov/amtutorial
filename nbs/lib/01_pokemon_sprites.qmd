...
It's both easier and faster to use HuggingFace to download the data than to scrape the pokemon website every time.

```{python}
#| export
def upload_pokemon_to_hf(repo_id: str, token: str = None):
    """Upload Pokemon sprites to Hugging Face Hub"""
    from huggingface_hub import HfApi, upload_file
    
    # Get the data files
    input_dir = get_cache_dir("data/pokemon/processed_adaptive")
    pixels_path = input_dir / 'pokesprites_pixels.npy'
    names_path = input_dir / 'pokesprites_names.txt'
    ...
    if not pixels_path.exists():
        download_pokemon_sprites()
        process_pokemon_sprites("processed_adaptive", binarize_method='adaptive')
        datafy_pokemon_sprites("processed_adaptive")
    
    # Create repo
    api = HfApi(token=token)
    api.create_repo(repo_id, repo_type="dataset", exist_ok=True)
    
    # Upload files (call upload_file directly, not as method)
    upload_file(
        path_or_fileobj=str(pixels_path),
        path_in_repo="pokesprites_pixels.npy", 
        repo_id=repo_id,
        repo_type="dataset",
        token=token
    )
    upload_file(
        path_or_fileobj=str(names_path),
        path_in_repo="pokesprites_names.txt",
        repo_id=repo_id, 
        repo_type="dataset",
        token=token
    )
...
@ft.lru_cache()
def download_pokemon_from_hf(repo_id: str):
    """Download Pokemon sprites from Hugging Face Hub (cached automatically)"""
    from huggingface_hub import hf_hub_download
    
    # HF automatically caches these files - no need to manage cache ourselves!
    pixels_path = hf_hub_download(repo_id, "pokesprites_pixels.npy", repo_type="dataset")
    names_path = hf_hub_download(repo_id, "pokesprites_names.txt", repo_type="dataset")
    
    poke_pixels = np.load(pixels_path)
    poke_pixels = (poke_pixels > 0).astype(np.float32) * 2 - 1
    poke_names = [n.strip().replace('.png', '') for n in open(names_path).readlines()]
    
    return poke_pixels, poke_names
```

Hugging Face automatically handles all the caching --- files are downloaded once and reused on subsequent calls.

```{python}
## Uploading only
# import os
# os.environ["HF_TOKEN"] = "your_token_here"
# upload_pokemon_to_hf("bhoov/pokemon-sprites-bipolar", os.environ["HF_TOKEN"])
