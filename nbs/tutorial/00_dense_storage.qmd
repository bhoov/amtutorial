...
The Classical Hopfield Network (CHN) [@hopfield1982neural] defines an energy function for this collection of patterns, putting the $\mu$-th stored pattern $\xi_\mu$ at a *low* value of energy. The CHN energy is a quadratic function described by dot-product correlations:
...
$$
\begin{align*}
E_\text{eDAM}(x) &= -\log \sum_{\mu=1}^K \exp \left(\beta \sum_{i=1}^D \xi_{\mu i} x_i\right)
\end{align*}
...
if os.path.exists(npz_fname) and CACHE_RECALL:
    ...
    x_final, frames, energies = jax.vmap(ft.partial(cached_recall, nsteps=16000, save=False), in_axes=(None, None, 0,0))(edam, cache_name, x_noisy, keys)
