...
Our goal in this section is to build the smallest abstraction for Associative Memory, which at its core is just an *energy function* $E_\Xi(x) \in \mathbb{R}$. where *query pattern* $\sigma \in \{-1, 1\}^D$ is a possibly noisy $D$-dimensional, binary pattern and *memory matrix* $\Xi \in \{-1, 1\}^{K \times D}$ is our matrix of $K$ stored patterns. $E_\Xi(\sigma)$ stores patterns at low energies. To retrieve our stored patterns,we want to minimize $E_\Xi(\sigma)$.
...
$$
E_\text{CHN}(\sigma) = -\frac{1}{2} \sum_\mu \left(\sum_{i} \xi^\mu_i x_i\right)^2 = -\frac{1}{2} \sum_{i,j} T_{ij} \sigma_i \sigma_j.
$${#eq-chn-energy}
