...
class EnergyTransformer(eqx.Module):
  ...
  @classmethod
  def rand_init(cls, key, config: ETConfig):
...
Note that the `xhat` inputs above are all layer-normalized tokens. However, like other AMs, we restrict ourselves to using non-linearties that are gradients of a convex Lagrangian function. Our "special layernorm"  is the same as the standard layer normalization *except* that we need our learnable `gamma` parameter to be a scalar instead of a vector of shape `D`. We will just show this in code below.
...
def normalize_img(x):
  x = (np.array(x) - IMAGENET_MEAN) / IMAGENET_STD
  return rearrange(x, "... h w c -> ... c h w")

def unnormalize_img(x):
  x = rearrange(np.array(x), "... c h w -> ... h w c")
  return ((x * IMAGENET_STD) + IMAGENET_MEAN).astype(np.uint8)

@ft.lru_cache
def get_normalized_imgs():
  imgs = np.stack([imageio.imread(f) for f in sorted(glob("./assets/et-figs/*.png"))])
...
unnormalized_imgs = unnormalize_img(imgs[:nh*nw])
...
We can process an images and batches of imags into tokens and back.
...
class ImageEnergyTransformer(eqx.Module):
  ...
  def decode(self, x):
    ...
    return self.patcher.untokenify(x) # (..., C, H, W)
...
ET has publicly available pretrained weights that can be used for masked-image inpainting. The model itself is pretty small ~20MB. We load the state dict from a saved .npz file as follows:
...
def get_pretrained_et():
  ...
  et_config = ETConfig(D=D, H=H, Y=Y, M=M)
  et = EnergyTransformer(
...
So can we inpaint images with ET?
...
og_img = imgs[0]
og_img_show = unnormalize_img(og_img)
...
# Show masked input, encoded & decoded but no ET
x = iet.encode(og_img) # Img to embedded tokens
x = iet.prep_tokens(x, mask)[1:]  # N+1,D
masked_img = iet.decode(iet.lnorm(x))
masked_img_show = unnormalize_img(masked_img)

# Show reconstructed input
recons = iet(og_img, mask, nsteps=12, step_size=0.1)
recons_img = unnormalize_img(recons)
...
ax = axs[0]
ax.imshow(masked_img_show / 255.)
ax.set_title("Masked Input")

ax = axs[1]
ax.imshow(recons_img / 255.)
ax.set_title("Reconstruction")

ax = axs[2]
ax.imshow(og_img_show / 255.)
