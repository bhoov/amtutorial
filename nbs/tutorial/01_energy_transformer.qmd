...
def energy_recall(
  Efn, # Scalar-valued energy function
  x_init, # Initial tokens
  nsteps, # Number of gradient descent steps
...
x_init = layer_norm(jr.normal(key3, (N, D))) # Layer normalized tokens
final_xhat, energy_history = energy_recall(eattn.energy, x_init, nsteps=3000, step_size=0.5)
```

```{python}
#| echo: false
#| label: fig-attn-energy-descent
...
We will implement the Hopfield Network energy in the following code, leaving the gradient updates to autograd.
