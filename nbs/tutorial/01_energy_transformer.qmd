...
class ImageEnergyTransformer(eqx.Module):
  ...
  def __call__(self, x: jnp.ndarray, mask: jnp.ndarray, nsteps=12, step_size=0.1):
    """A complete pipeline for masked image modeling in ET using `energy_recall`
    
    Args:
      return_intermediates: If True, return all intermediate xhat values from gradient descent
    """
    x = self.encode(x)
    ...
    return self.decode(xhat_final), traj_outputs
...
```{python}
def inpaint_image(et, img, n_mask, key, nsteps=12, step_size=0.1):
    ...
    x = iet.encode(img)  # Img to embedded tokens
...
def show_et_recall_animation(traj_outputs, cache_name, steps_per_sample=1, force_remake=False, fps=2):
    "Create animated video showing both image reconstruction evolution and energy descent"
    video_fname = CACHE_DIR / (cache_name + ".mp4")
    def decode_to_show(xhat): return unnormalize_img(iet.decode(xhat[1:])) # Drop CLS
...
video, video_fname = show_et_recall_animation(traj_outputs, "et_reconstruction", 
...
video, video_fname = show_et_recall_animation(traj_outputs, "et_reconstruction_long", 
                                             steps_per_sample=1, force_remake=True)
Markdown(f"![]({video_fname})")
```
